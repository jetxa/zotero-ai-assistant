...
class StorageManager {
        ...
        for (const id of attachmentIDs) {
            ...
            if (title.startsWith("AI-Assistant-")) {
                try {
                    ...
                    Zotero.debug(`[AI Assistant] Failed to load session from attachment ${id}: ${e}`);
                }
            }
        }
        return sessions;
    ...
    static async saveSession(parentItem: Zotero.Item, session: ChatSession): Promise<void> {
        try {
...
export class ChatManager {
  ...
  static onAction: ((text: string, action: "translate" | "explain") => Promise<void>) | null = null;

  static async handleExternalAction(text: string, action: "translate" | "explain") {
      if (this.onAction) {
          await this.onAction(text, action);
      } else {
          ztoolkit.getGlobal("alert")("Please open the AI Assistant sidebar first.");
      }
  }
  ...
  static registerChatSection() {
    ...
    Zotero.ItemPaneManager.registerSection({
      ...
      onRender: async ({ body, item }) => {
        ...
        let container = body.querySelector("#zotero-llm-chat-main") as HTMLElement;
        if (!container) {
            ...
            toolbar.style.borderBottom = "1px solid var(--material-divider, #ccc)";
            toolbar.style.marginBottom = "5px";
            container.appendChild(toolbar);

            const sessionSelect = doc.createElementNS(NS, "select") as HTMLSelectElement;
            ...
            // Messages
            const messagesContainer = doc.createElementNS(NS, "div") as HTMLElement;
            ...
            messagesContainer.style.border = "1px solid var(--material-divider, #ccc)";
            messagesContainer.style.padding = "10px";
            messagesContainer.style.borderRadius = "4px";
            messagesContainer.style.backgroundColor = "var(--material-background, #f9f9f9)";
            ...
            const input = doc.createElementNS(NS, "textarea") as HTMLTextAreaElement;
            ...
            input.style.backgroundColor = "var(--material-side-bar-background, #fff)";
            ...
            const fullTextContainer = doc.createElementNS(NS, "div") as HTMLElement;
            ...
            const fullTextCheckbox = doc.createElementNS(NS, "input") as HTMLInputElement;
            ...
            const fullTextLabel = doc.createElementNS(NS, "label") as HTMLLabelElement;
            ...
            const sendBtn = doc.createElementNS(NS, "button") as HTMLButtonElement;
            sendBtn.id = "zotero-llm-chat-send";
            sendBtn.textContent = "Send";
            sendBtn.style.flex = "1";
            sendBtn.style.cursor = "pointer";
            sendBtn.style.backgroundColor = "var(--material-side-bar-background, #eee)";
            sendBtn.style.border = "1px solid var(--material-divider, #ccc)";
            sendBtn.style.borderRadius = "4px";
            rightControls.appendChild(sendBtn);
        }
        
        const sendBtn = body.querySelector("#zotero-llm-chat-send") as HTMLButtonElement;
        const newBtn = body.querySelector("#zotero-llm-chat-new-btn") as HTMLButtonElement;
        const sessionSelect = body.querySelector("#zotero-llm-chat-session-select") as HTMLSelectElement;
        const fullTextCheckbox = body.querySelector("#zotero-llm-chat-fulltext-checkbox") as HTMLInputElement;
        const input = body.querySelector("#zotero-llm-chat-input") as HTMLTextAreaElement;
        const messagesContainer = body.querySelector("#zotero-llm-chat-messages") as HTMLDivElement;
        ...
        const renderMessages = (messages: ChatMessage[]) => {
            ...
            if (messages.length === 0) {
                 messagesContainer.innerHTML = '<div id="zotero-llm-chat-placeholder" style="opacity: 0.7; font-style: italic; text-align: center; margin-top: 10px;">Start a new conversation!</div>';
                 return;
            }
            messages.forEach(msg => appendMessageUI(messagesContainer, msg.role, msg.content, msg.tokenCount, msg.promptTokenCount));
        };
        ...
        const loadSessions = async () => {
            ...
            if (currentSessions.length > 0) {
                ...
                currentSession = currentSessions.find(s => s.id === lastId) || currentSessions[0];
        ...
        ChatManager.onAction = async (text: string, action: "translate" | "explain") => {
            const defaultTranslatePrompt = "Please translate the following text into Chinese:";
            const defaultExplainPrompt = "Please explain the following text:";
            let template = "";
            if (action === "translate") {
                template = (getPref("promptTranslate" as any) as string) || defaultTranslatePrompt;
            } else if (action === "explain") {
                template = (getPref("promptExplain" as any) as string) || defaultExplainPrompt;
            }
            if (!template || template === "undefined") {
                template = action === "translate" ? defaultTranslatePrompt : defaultExplainPrompt;
            }
        ...
        sendBtn.onclick = async () => {
            ...
            try {
                ...
                if (currentSession!.messages.length === 1) {
                    currentSession!.title = question.substring(0, 20) + (question.length > 20 ? "..." : "");
                ...
                if (fullTextCheckbox && fullTextCheckbox.checked) {
                    ...
                    if (fullText) {
                        const limit = (getPref("fullTextLimit" as any) as number) || 100000;
                        context += `\n\n[Full Text Content]:\n${fullText.substring(0, limit)}`; 
                    }
                }
                loadingBubble = appendMessageUI(messagesContainer, "assistant", "Thinking...");
...
function appendMessageUI(container: HTMLElement, role: string, text: string, tokenCount?: number, promptTokenCount?: number) {
  ...
  const timestamp = doc.createElementNS("http://www.w3.org/1999/xhtml", "div") as HTMLElement;
  ...
  if (role === "assistant" && (tokenCount !== undefined || promptTokenCount !== undefined)) {
    const tokenDisplay = doc.createElementNS("http://www.w3.org/1999/xhtml", "div") as HTMLElement;
    ...
    const inputStr = promptTokenCount !== undefined ? `Input: ${promptTokenCount}` : "";
    const outputStr = tokenCount !== undefined ? `Output: ${tokenCount}` : "";
    tokenDisplay.textContent = [inputStr, outputStr].filter(s => s).join(" | ") + " tokens";
