...
export class ReaderManager {
    static init() {
        Zotero.Reader.registerEventListener("renderTextSelectionPopup", (event) => {
            ...
            Zotero.debug(`[AI Assistant] Popup rendered. Text from params: "${text?.substring(0, 30)}..."`);
            ...
            const createBtn = (label: string, action: "translate" | "explain") => {
                ...
                btn.onclick = async (e) => {
                    ...
                    try {
                        if (text) {
                             Zotero.debug(`[AI Assistant] Calling ChatManager.handleExternalAction...`);
                             if (typeof ChatManager === 'undefined') {
                                 Zotero.debug(`[AI Assistant] Error: ChatManager is undefined!`);
                             } else {
                        ...
                        Zotero.debug(`[AI Assistant] Error in button click handler: ${error}`);
